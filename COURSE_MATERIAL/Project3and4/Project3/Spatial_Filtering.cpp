...
Mat Spafilt::geoMeanFilter(Mat & image_in, int sizeOfFilter_in)
{
	this->setImage(image_in);
	this->filterSize = sizeOfFilter_in;
	this->borderProcessing();
		...
		for (int counter1 = 0; counter1 < image_out.rows; counter1++)
		{
			for (int counter2 = 0; counter2 < image_out.cols; counter2++)
			{
				...
				for (int counter_1 = counter1; counter_1 < this->filterSize + counter1; counter_1++)
				{
					for (int counter_2 = counter2; counter_2 < this->filterSize + counter2; counter_2++)
					{
						double temp = this->imageAfterBorderProcess.at<uchar>(counter_1, counter_2);
						if (temp == 0)
							temp = 128;
						sum *= temp;
			...
			for (int counter1 = 0; counter1 < image_out.rows; counter1++)
			{
				for (int counter2 = 0; counter2 < image_out.cols; counter2++)
				{
					...
					for (int counter_1 = counter1; counter_1 < this->filterSize + counter1; counter_1++)
					{
						for (int counter_2 = counter2; counter_2 < this->filterSize + counter2; counter_2++)
						{
							double temp= this->imageAfterBorderProcess.at<Vec3b>(counter_1, counter_2)[channel];
							if (temp == 0)
								temp = 1;
							sum *= temp;
					...
					sum = pow(sum, 1.0 / 25);
					image_out.at<Vec3b>(counter1, counter2)[channel] = sum;
				}
			}
		}
		return image_out;
...
Mat Spafilt::harMeanFilter(Mat & image_in, int sizeOfFilter_in)
{
	this->setImage(image_in);
	this->filterSize = sizeOfFilter_in;
	this->borderProcessing();
		...
		for (int counter1 = 0; counter1 < image_out.rows; counter1++)
		{
			for (int counter2 = 0; counter2 < image_out.cols; counter2++)
			{
				...
				for (int counter_1 = counter1; counter_1 < this->filterSize + counter1; counter_1++)
				{
					for (int counter_2 = counter2; counter_2 < this->filterSize + counter2; counter_2++)
					{
						int temp = this->imageAfterBorderProcess.at<uchar>(counter_1, counter_2);
						if (temp == 0)
							temp = 128;
						sum += 1.0/temp;
...
Mat Spafilt::conMeanFilter(Mat & image_in, int sizeOfFilter_in,int q)
{
	this->setImage(image_in);
	this->filterSize = sizeOfFilter_in;
	this->borderProcessing();
...
Mat Spafilt::adaptive_median_filte(const Mat & image_in, int sizeOfFilter_in)
{
	Mat img_filtered= image_in.clone();

	int nRows = image_in.rows;
	int nCols = image_in.cols;

	int offset = sizeOfFilter_in / 2;
	int ttl = sizeOfFilter_in * sizeOfFilter_in;
	int beginR = offset, beginC = offset;
	int endR = nRows - offset, endC = nCols - offset;
	int window_size = 3, window_size_max = sizeOfFilter_in;
	...
	for (int i = beginR; i < endR; i++) 
	{
		for (int j = beginC; j < endC; j++) 
		{
			...
			{
				sizeOfFilter_in = window_size;
				offset = sizeOfFilter_in / 2;
				vector<int> val_list;
				for (int x = 0; x < sizeOfFilter_in; x++)
				{
					for (int y = 0; y < sizeOfFilter_in; y++) 
					{
						int val_img = int(image_in.at<uchar>(i + x - offset, j + y - offset));
				...
				{
					// B
					int Zxy = int(image_in.at<uchar>(i, j));
					if ((Zxy - Zmin) > 0 && (Zxy - Zmax) < 0)
					{
						img_filtered.at<uchar>(i, j) = uchar(Zxy);
					}
					else 
					{
						img_filtered.at<uchar>(i, j) = uchar(Zmed);
					}
					break;
				}
...
Mat Spafilt::adaptive_mean_filte(const Mat &image_in, int sizeOfFilter_in, double stdev)
{
	Mat img_filtered = image_in.clone();
	int nRows = image_in.rows;
	int nCols = image_in.cols;
	...
	for (int i = beginR; i < endR; i++) 
	{
		for (int j = beginC; j < endC; j++) 
		{
			...
			for (int x = 0; x < sizeOfFilter_in; x++)
			{
				for (int y = 0; y < sizeOfFilter_in; y++) 
				{
					uchar val_img = image_in.at<uchar>(i + x - offset, j + y - offset);
			...
			variance_local /= 10000;

			double val = double(image_in.at<uchar>(i, j));

			if (variance_local != 0) 
			{
				val -= (val - mean) * variance_global / variance_local;
			}
			img_filtered.at<uchar>(i, j) = uchar(val);
		}
	}
...
Mat Spafilt::addSaltAndPepperNoise(const Mat &image_in, float rate)
{
	...
	else
	{
		for (int counter = 0; counter < numberOfNoise; counter++)
		{
			...
			if (counter % 2 == 0)
			{
				image_out.at<Vec3b>(row, col)[0] = 255;
				image_out.at<Vec3b>(row, col)[1] = 255;
				image_out.at<Vec3b>(row, col)[2] = 255;

			}
			else
			{
				image_out.at<Vec3b>(row, col)[0] = 0;
				image_out.at<Vec3b>(row, col)[1] = 0;
				image_out.at<Vec3b>(row, col)[2] = 0;
			}
		}
		return image_out;
	}
