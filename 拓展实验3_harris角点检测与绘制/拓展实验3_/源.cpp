...
 2.计算出Ix^2,Iy^2,Ix*Iy ：
  concer:在水平和竖直两个方向上变化均较大的点，即lx,ly都较大
  edge:仅在水平和竖直方向有较大的点，即lx,ly只有其一较大
  flat:仅在水平，竖直方向的变化量均较小的点，即lx,ly都较小

  3.利用高斯函数对Ix^2,Iy^2,Ix*Iy进行滤波

  4.计算局部特征结果矩阵M的特征值和响应函数C(i,j)=Det(M)-k(trace(M))^2   (0.04<=k<=0.06) 
  角点相应函数 R为大数值正数，边缘R为大数值负数，平坦区绝对值R是小数值
...
int main()
{
	const Mat srcImage = imread("G:\\Code\\Digital_image_processing\\images\\6.jpg");
	if (!srcImage.data)
	{
